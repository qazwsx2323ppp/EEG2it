...
# Initialize EEG -> SD image projector
sd_hidden_size = painter.pipe.text_encoder.config.hidden_size
# Map EEG image embedding (512) -> SD cross-attn dim
# NOTE: this projector is untrained by default
# Optional: load weights via EEG_IMG_PROJ_CKPT env var
_eeg_img_proj = torch.nn.Linear(512, sd_hidden_size).to("cuda").half()
_eeg_img_proj_ckpt = os.environ.get("EEG_IMG_PROJ_CKPT", "").strip()
if _eeg_img_proj_ckpt:
    if not os.path.exists(_eeg_img_proj_ckpt):
        raise FileNotFoundError(f"EEG_IMG_PROJ_CKPT not found: {_eeg_img_proj_ckpt}")
    _eeg_img_proj.load_state_dict(torch.load(_eeg_img_proj_ckpt, map_location="cpu"))
...
try:
    ...
    with torch.no_grad():
        emb_img, _, _ = eeg_encoder(dummy_eeg.to("cuda").half())

    image = painter.generate_with_eeg(
        prompt=response,
        eeg_img_emb=emb_img,
        eeg_proj=_eeg_img_proj,
