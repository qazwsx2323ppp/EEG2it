"""
DDPM / LDM 采样与训练核心模块。
- 基于高斯扩散的 DDPM（图像空间）与 Latent Diffusion（潜空间）实现
- 提供时间表、前向/后验分布、训练损失、采样循环等
- 在本项目中，结合条件编码器（如 EEG 的特征）进行条件生成
...
class DDPM(pl.LightningModule):
    """
    经典 DDPM：在图像空间进行高斯扩散的训练与采样。
    功能：
    - 注册扩散时间表与系数（register_schedule）
    - 前向/后验分布计算（q_mean_variance, q_posterior）
    - 单步/整段采样（p_sample, p_sample_loop, sample）
    - 训练损失与前向过程（p_losses, forward）
    """
    ...
    def register_schedule(self, given_betas=None, beta_schedule="linear", timesteps=1000,
        ...
        """
        构建并注册扩散时间表与相关系数。
        - 支持线性/余弦等 beta 调度或直接给定 betas
        - 预计算并缓存 alpha 累乘、后验均值/方差、LVLB 权重等
        """
    ...
    def q_mean_variance(self, x_start, t):
        ...
        前向扩散分布 q(x_t | x_0)。
        参数:
        - x_start: 干净输入 [N,C,H,W]
        - t: 时间步索引
        返回:
        - mean, variance, log_variance（与 x_start 同形状）
        """
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool):
        """
        基于模型输出得到后验均值与方差，用于反向去噪一步。
        - 参数化为 eps 时：先估计 x0，再计算后验
        - 参数化为 x0 时：模型直接输出 x0
        """
    ...
    def p_sample(self, x, t, clip_denoised=True, repeat_noise=False):
        """
        单步采样：从时刻 t 的 x 生成 t-1 的样本。
        - 使用后验均值与方差，加噪声（t>0 时）得到下一步
        """
    ...
    def sample(self, batch_size=16, return_intermediates=False):
        """
        简便采样入口：根据配置的图像大小与通道，调用 `p_sample_loop`。
        """
    ...
    def q_sample(self, x_start, t, noise=None):
        """
        前向加噪：构造 x_t = sqrt(alpha_cumprod)*x0 + sqrt(1-alpha_cumprod)*noise。
        """
...
class LatentDiffusion(DDPM):
    """
    潜空间扩散（LDM）：在自动编码器的潜空间进行扩散与采样。
    功能：
    - 第一阶段模型（VQ/VAE）编码/解码（encode/decode_first_stage）
    - 条件阶段模型（EEG/文本/图像等）用于条件生成（instantiate_cond_stage, get_learned_conditioning）
    - 与 DDPM 相同的训练/采样接口，但在潜空间进行，提高效率与质量
    在本项目中：使用 EEG 条件向量驱动潜空间扩散，结合图像嵌入对齐（FrozenImageEmbedder）提升感知一致性。
    """
    def __init__(self,
                first_stage_config,
                cond_stage_config,
