...
class TripletDataset(Dataset):
    ...
    def __init__(self, cfg_data, mode='train'):
        ...
        try:
            # raw_indices 包含指向 all_eeg_items 列表的原始索引
            raw_indices = splits_data['splits'][0][mode]
            print(f"为 {mode} 模式找到了 {len(raw_indices)} 个原始 EEG 条目索引。")
        except (KeyError, IndexError) as e:
            print(f"加载 splits 文件时出错: {e}. 请检查 {cfg_data.splits_path} 的结构。")
            raise
