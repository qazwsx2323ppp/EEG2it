...
from bridge import build_eeg_tower
...
#################
        self.eeg_tower = build_eeg_tower(
            hidden_size=config.text_config.hidden_size,
            num_queries=getattr(config, "eeg_num_queries", 32),
            backbone_kwargs=dict(
                time_len=getattr(config, "eeg_time_len", 512),
                in_chans=getattr(config, "eeg_in_chans", 128),
                embed_dim=getattr(config, "eeg_embed_dim", 1024),
                depth=getattr(config, "eeg_depth", 24),
                num_heads=getattr(config, "eeg_num_heads", 16),
                mlp_ratio=getattr(config, "eeg_mlp_ratio", 1.0),
                global_pool=getattr(config, "eeg_global_pool", False),
            ),
        )
...
##########################
    ...
    def forward(
        ...
        eeg_token_index: Optional[int] = None,
        ...
        if input_ids is not None and input_ids.shape[1] != 1:  # Prefill stage
            ...
            if eeg_values is not None and eeg_token_index is not None:
                self.eeg_tower.to('cuda')
                eeg_embeds = self.eeg_tower(eeg_values)
                eeg_mask = (
                    (input_ids == eeg_token_index)
                    .unsqueeze(-1)
                    .expand_as(inputs_embeds)
                    .to(inputs_embeds.device)
                )
                eeg_embeds = eeg_embeds.to(inputs_embeds.device, inputs_embeds.dtype)
                inputs_embeds = inputs_embeds.masked_scatter(eeg_mask, eeg_embeds)
                self.eeg_tower.to('cpu')
                torch.cuda.empty_cache()
