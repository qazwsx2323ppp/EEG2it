...
print(f"向量总数: {len(img_vecs)}")
print(f"向量维度: {img_vecs.shape}")

# 加载一个 EEG 数据看看里面的 index 是什么样
# (假设你用 dataset_2o.py 里的逻辑)
data = torch.load("data/EEG_data/eeg_55_95_std.pth")
dataset = data['dataset']
print(f"样本 0 的图片索引: {dataset[0]['image']}")
print(f"样本 100 的图片索引: {dataset[100]['image']}")

import torch
data = torch.load('data/EEG_data/eeg_55_95_std.pth')
sample = data['dataset'][0]
print("EEG样本包含的键:", sample.keys())
# 重点看有没有 'image_path', 'image_name' 或者类似的字段

# 关键检查：
# 如果样本 0 的索引是 5，请确认 img_vecs[5] 真的是这张图的向量。
# 如果没办法确认，这就是最大的风险点。